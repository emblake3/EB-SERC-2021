...
g6 <- ggplot(data = barGraphStats(data = damage5, variable = "abundance", byFactorNames = 
  c("Area_type", "insectgroup")), aes(x=Area_type, y=mean, fill=insectgroup)) +
   geom_bar(stat='identity', position=position_dodge()) +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width = 0.2, position = position_dodge(0.9)) +        xlab("Area Type") + ylab("Abundance") + theme(legend.position = "none")

g6 + theme(panel.background = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), legend.title=element_blank(), legend.text=element_text(size=18))
